...
BEGIN
    ...
    IF EXISTS (SELECT `fk_userID` FROM `zeeorder_details` WHERE `zl_status`='C' AND `fk_userID`=_userid) THEN 
        SELECT
            ...
            MAX(CASE WHEN current_date() BETWEEN date(offer.`FromDate`) AND date(offer.`ToDate`)
                 ...
                 THEN 
                     JSON_OBJECT('offerid', offer.`id`, 'offername', offer.`name`, 'offeredproducts',  FLOOR(offer.`offerQty` * (zd.`zl_count` / offer.`eligibilityQty`)))
                 ELSE NULL
