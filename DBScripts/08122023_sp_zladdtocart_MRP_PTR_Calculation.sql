...
BEGIN
    ...
    DECLARE totalAmount DECIMAL(10, 2); -- Assuming a decimal type for total amount

    IF _ptr IS NULL OR _ptr = '' THEN
        SET totalAmount = CAST(_count AS DECIMAL(10, 2)) * CAST(_mrp AS DECIMAL(10, 2));
    ELSE
        SET totalAmount = CAST(_count AS DECIMAL(10, 2)) * CAST(_ptr AS DECIMAL(10, 2));
    END IF;
    ...
    IF EXISTS(SELECT `userId` FROM `zeeuser_login` WHERE `userId` = _userid) THEN
        IF NOT EXISTS (SELECT `zl_product_id` FROM `zeeorder_details` WHERE `zl_product_id` = _proid AND `fk_userID` = _userid) THEN
            ...
            ELSE
                ...
                VALUES (_userid, _productName, _count, _mrp, 'C', NOW(), totalAmount, _proid);
            ...
            ELSE
                ...
                ELSE
                    UPDATE `zeeorder_details` SET
                        ...
                        `zl_totalAmnt` = (updatedCount * totalAmount)
