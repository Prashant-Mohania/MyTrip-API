CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_updateProfile`(_userID varchar(10),_userName varchar(50),_storeName varchar(50),
...
BEGIN
              if exists(select `email` from `zeeuser_login` where `status`='1' and `userId`=_userID) then
				...
				set `userName`=_userName,`storeName`=_storeName,
