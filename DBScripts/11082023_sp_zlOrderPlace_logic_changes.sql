...
where t2.`fk_userID`=_userid and t2.`zl_status` = 'C';	
	...
	WHERE
		`fk_userID` = _userid and `zl_status` = 'C';
					
					INSERT INTO `zeeorder_track`(`fk_userID`,`zl_orderID`,`zl_orderDate`,`zl_comment`,`zl_totaloforder`,`SAPOrderStatusId`) VALUES
				(_userid,_orderID,NOW(),'Placed',_mrp,'1');
