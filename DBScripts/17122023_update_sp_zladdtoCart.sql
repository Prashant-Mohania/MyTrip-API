...
BEGIN
    ...
    IF EXISTS(SELECT `userId` FROM `zeeuser_login` WHERE `userId` = _userid) THEN
        IF NOT EXISTS (SELECT `zl_product_id` FROM `zeeorder_details` WHERE `zl_product_id` = _proid AND `fk_userID` = _userid) THEN
            ...
            ELSE
                INSERT INTO `zeeorder_details`(`fk_userID`, `zl_productName`, `zl_count`, `zl_mrp`, `zl_status`, `zl_indate`, `zl_totalAmnt`, `zl_product_id`, `ptr`, `gst`,`offer_qty`)
                VALUES (_userid, _productName, _count, _mrp, 'C', NOW(), totalAmount, _proid,_ptr,_gst,_offer_qty);
