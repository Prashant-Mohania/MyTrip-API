...
BEGIN
    DECLARE _Totaloforder DECIMAL(30, 6) DEFAULT 0;
    DECLARE _orderID VARCHAR(200);
    
    -- Assuming ZEEORDERID returns a unique order ID based on some logic
    SELECT ZEEORDERID(_userid) INTO _orderID;

    -- Check if there are products in the user's cart
    IF EXISTS (SELECT 1 FROM `zeeorder_details` WHERE `fk_userID` = _userid AND `zl_status` = 'C') THEN
        -- Update productlist OnHand
        UPDATE `productlist` AS t1
        JOIN `zeeorder_details` AS t2 ON t1.`pk_product_id` = t2.`zl_product_ID_int`
        ...
        SELECT SUM(zd.`gst` + zd.`zl_totalAmnt`) INTO _Totaloforder
        ...
        VALUES (_userid, _orderID, NOW(), 'Placed', _Totaloforder, '1');

        SELECT 'Y' AS 'id', 'Order Placed Successfully!' AS 'Result';

    ELSE
        SELECT 'N' AS 'id', 'No products found in the cart' AS 'Result';
    END IF;
END